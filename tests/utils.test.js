...
it("should find the perimiter", function() {
    const perimiter = utils.perimiter(5, 6)
    expect(perimiter).to.be.a("number")
    expect(perimiter).to.equal(22)
})

it("should find area of a circle", function() {
    const circleArea = utils.circleArea(2)
    expect(circleArea).to.be.a("number")
    expect(circleArea).to.equal(MATH.PI ** 2)
})
